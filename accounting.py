def check_log_for_underpaid_sales(melon_cost):
    orders_log = open('customer_orders.csv')

    for line in orders_log:
        line = line.rstrip()
        customer_info_list = line.split(',')
        customer_name, num_melons_purchased, amount_paid = customer_info_list[1:]

        pay_expected = float(num_melons_purchased) * float(melon_cost)
        amount_paid = float(amount_paid)
        if pay_expected != amount_paid:
            print customer_name, "paid %.2f, expected %.2f"%(amount_paid, pay_expected)
...
def main():
    ...
    check_log_for_underpaid_sales(melon_cost)
